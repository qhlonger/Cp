...
- (void)layout{
    ...
    [self.moneyTf mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.bgRedImgView);
        make.top.equalTo(self.view).offset(PtOn40(15));
        make.height.mas_equalTo(RowH);
    }];
    [self.submitBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.top.bottom.equalTo(self.moneyTf);
        make.width.mas_equalTo(PtOn47(80));
    }];
    [self.bgRedImgView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.view);
        make.top.equalTo(self.moneyTf.mas_bottom).offset(PtOn47(7));
    ...
    [self.helpLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.bgRedImgView.mas_bottom).offset(PtOn47(0));
        make.centerX.equalTo(self.view);
...
- (UIButton *)submitBtn{
    if (!_submitBtn) {
        ...
        btn.titleLabel.font = CPFont(16);
        [btn bk_addEventHandler:^(id sender) {
            
        } forControlEvents:UIControlEventTouchUpInside];
        [self.moneyTf addSubview:btn];
        _submitBtn = btn;
    }
    return _submitBtn;
}
...
- (YYLabel *)helpLabel{
    if(!_helpLabel){
        ...
        label.textContainerInset = UIEdgeInsetsMake(PtOn47(0), PtOn47(20), PtOn47(0), PtOn47(20));
